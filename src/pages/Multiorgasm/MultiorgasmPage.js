...
import Intimos from "../../assets/images/Banner-Multiorgasmos.png";

import "./Multiorgasm.scss";
...
const IntimosPage = () => {
  ...
  const IntimosProducts = products.filter(
    (product) => product.title === "Multiorgasmos"
  );

  return (
    <>
      <Banner image={Intimos} />
      <Categories />
      <section className="py-5 no-pt">
        <div className="container">
          <Title title="Multiorgasmos" />
          <div className="row">
            {IntimosProducts.map((product) => {
              return (
                <div
                  key={product.id}
